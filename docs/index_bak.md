...
mesh functions, which store the nodes, cell topology and additional cell attributes.


The mesh in TipiFEM is stored in an object of type `Mesh` that stores `MeshFunction`
objects
...
and a set of cells ${\cal M}$, where each cell has again a unique identifier and
set of vertices.
...
also do not store any information. A concrete `MeshCell` (e.g. a `Polytope`) is therefore
...
an initialization procedure, has the be run only once. Here we assign each cell type
a disjoint integer range as large as the number of cells of this type in the mesh.
Now every

implemented by first assigning disjoint
integer ranges to each cell type and useing the starting point of each interval as an offset
